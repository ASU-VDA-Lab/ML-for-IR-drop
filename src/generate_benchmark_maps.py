...
#################################################################################################
  ...
  #designs = [ "aes", "bp", "bp_be", "bp_fe", "bp_multi",
  #            "ibex", "jpeg", "swerv", "swerv_wrapper" ]
...
####################################################################################################
 ...
 #############################################################################################
  ...
  #generate_outputs(gan_current_maps[1400:1500], gan_designs[1400:1500],location_results,
  #                 technology, True )
...
def generate_outputs(current_maps, designs, location_results, technology, generated=False):
  ...
  for n, current_map in enumerate(pbar):
    ...
    res_fname = location_results  + '/%s/%s/%s_current_map.csv'%(technology,
                                                                 folder,
                                                                 designs[n])
    np.savetxt(res_fname, current_map, fmt='%5.3e', delimiter=',')
...
def solve_ir(ir_solver_params,
  ...
  voltages = save_heat_map(solution, res_fname, False)
  plot_hist(solution[:,2], res_fname, num_bins=20,plot=False)
  return voltages, grid_h

def save_heat_map(vol_dok, res_fname, plot= False):
  ## target grid to interpolate to
  xi = np.arange(0,np.max(vol_dok[:,0]),1)
  yi = np.arange(0,np.max(vol_dok[:,1]),1)
