...
#  designs = [ "bp", "bp_multi"]
  ...
  seeds = {"aes":1, "bp":2, "bp_be":3, "bp_fe":4, "bp_multi":5, "dynamic_node":6,
           "ibex":7, "jpeg":8, "swerv":9, "swerv_wrapper":10} 
...
####################################################################################################
  ...
  design_current_maps, design_mean, design_areas = load_current_maps(technology,
                                                                     ...
                                                                     location_designs)
  print(design_mean)
  generate_outputs(design_current_maps, designs,location_results, technology,
  seeds,  False)
...
def generate_outputs(current_maps, designs, location_results, technology, seeds, generated=False):
  ...
  for n, current_map in enumerate(pbar):
    ...
    res_fname = location_results  + '/%s/%s/%s_current_reg'%(technology,
                                                             folder,
                                                             designs[n])
    save_and_plot(res_fname, current_map, plot)
    res_fname = location_results  + '/%s/%s/%s_current_irreg'%(technology,
                                                             folder,
                                                             designs[n])
    save_and_plot(res_fname, current_map, plot)
...
#    print('num_vsrc',len(vsrc_nodes))
    ...
    res_fname = location_results  + '/%s/%s/%s_voltage_reg'%(technology,
                                                                     folder,
                                                                     designs[n])
    ...
    res_fname = location_results  + '/%s/%s/%s_eff_dist_reg'%(technology,folder,designs[n])
    generate_eff_dist_map(res_fname,grid_params,vsrc_nodes,plot)
    res_fname = location_results  + '/%s/%s/%s_eff_dist_irreg'%(technology,folder,designs[n])
    generate_eff_dist_map(res_fname,grid_params,vsrc_nodes,plot)

    pbar.set_postfix({'Status': '%15s'%'Generate regular spice'})
    res_fname = location_results  + '/%s/%s/%s_reg.sp'%(technology,folder,designs[n])
    ...
    res_fname = location_results  + '/%s/%s/%s_regions_reg'%(technology,folder,designs[n])
    generate_region_maps(res_fname, grid_params, regions, reg_state,plot)
    res_fname = location_results  + '/%s/%s/%s_regions_irreg'%(technology,folder,designs[n])
    generate_region_maps(res_fname, grid_params, regions, irreg_state,plot)
    
    pbar.set_postfix({'Status': '%15s'%'Generate irregular IR'})
    res_fname = location_results  + '/%s/%s/%s_voltage_irreg'%(technology,
                                                                       folder,
                                                                       designs[n])
    ...
    res_fname = location_results  + '/%s/%s/%s_irreg.sp'%(technology,
                                                               folder,
                                                               designs[n])
...
def solve_ir(ir_solver_params,
  ...
  voltages = save_heat_map(solution, res_fname, grid_params, plot)
